...
- **정기 발행 4회** (한국시간 09:00 / 13:00 / 17:00 / 21:00)
  - 따끈한 신규 소식이 없으면, 곧 예정된 발매/이벤트 일정을 미리 알리는 "예고" 포스트로 대체
    (미래 일정이라 최신성이 유지됨). 과거 뉴스 재탕·밈/트리비아 대체는 하지 않음
  - GitHub Actions 무료 Actions 사용량(월 2,000분) 절약을 위해 하루 4회로 제한. 자동 속보
    체크 cron은 두지 않고, 필요하면 Actions 탭에서 `mode=breaking`으로 수동 실행 가능

> ⚠️ 모드 판정은 실행 시각이 아니라 `github.event.schedule`(어느 cron이 트리거했는지)로 합니다.
> GitHub Actions 스케줄은 부하 시 최대 1시간 가까이 지연될 수 있는데, 실행 시각 기준으로 판정하면
> 지연된 정기 슬롯이 엉뚱한 모드로 잘못 분류되는 문제가 있어 이렇게 고쳤습니다.
...
```
GitHub Actions (하루 4회 정기 발행 트리거, 필요시 수동으로 breaking 실행)
        ↓
정기(scheduled)/속보(breaking) 모드 판별 (github.event.schedule 기준, 수동 실행은 입력값 사용)
        ↓
