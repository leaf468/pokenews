...
def build_user_prompt(mode: str, recent_topics: list[str]) -> str:
    ...
    if mode == "scheduled":
        ...
        instruction = (
            "지금 시각 기준 최근 몇 시간 이내에 포켓몬 카드 관련해서 화제가 될 만한 일이 "
            "있었는지 웹 검색으로 확인해줘. 예: 신제품 깜짝 발표, 초고가 경매 낙찰, "
            "대량 위조 카드 적발, 유명인 관련 화제, 품절 대란, 그리고 팝업스토어·전시회 같은 "
            "오프라인 이벤트 소식(지금 한창 진행 중인 것도 포함, 예: 잠실 팝업스토어), "
            "커뮤니티에서 화제가 된 개봉 영상/짤 등. "
            "그런 소식이 마땅치 않으면, 세상에서 제일 비싼 카드 경매 비하인드, 유명한 거래/사기 "
            "에피소드, 카드 역사 뒷이야기, 가품 구별 팁처럼 아무 때나 써먹을 수 있는 가벼운 "
            "가십으로 대체해도 좋아. 이 모드도 항상 결과를 만들어야 해 — 정말 아무 소재도 "
            "못 찾았을 때만 has_news를 false로 응답해."
        )
