...
def build_user_prompt(mode: str, recent_topics: list[str]) -> str:
    ...
    if mode == "scheduled":
        ...
        instruction = (
            ...
            "유명인 관련 화제, 품절 대란, 팝업스토어·전시회 같은 오프라인 이벤트 소식(지금 "
            "한창 진행 중인 것도 포함, 예: 잠실 팝업스토어), 커뮤니티에서 화제가 된 개봉 영상/짤 등.\n"
            "그런 화제성 소식을 못 찾았어도 실패로 취급하지 마. 그 경우엔 대신 세상에서 제일 "
            "비싼 카드 경매 비하인드, 유명한 거래/사기 에피소드, 카드 역사 뒷이야기, 가품 구별 "
            "팁처럼 아무 때나 써먹을 수 있는 가벼운 가십·트리비아를 골라서 Threads 포스트를 "
            "작성해. 이런 상시 소재는 검색 없이도 네가 이미 알고 있는 걸로 충분해.\n"
